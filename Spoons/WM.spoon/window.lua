...
local function snap(win, x, y, w, h, gaps, screenFrame)
  ...
  win:setFrame(utils.applyGaps(raw, f, gaps))
end
...
-- distribute windows into an even grid
...
function M:bind(cfg)
  ...
  local function s(win, x, y, w, hh)
    snap(win, x, y, w, hh, gaps, win:screen():frame())
  end

  local actions = {
    ["win.snapLeft"]    = function() s(utils.focused(), 0,   0,   0.5, 1)   end,
    ["win.snapRight"]   = function() s(utils.focused(), 0.5, 0,   0.5, 1)   end,
    ["win.snapTop"]     = function() s(utils.focused(), 0,   0,   1,   0.5) end,
    ["win.snapBottom"]  = function() s(utils.focused(), 0,   0.5, 1,   0.5) end,
    ["win.maximize"]    = function() s(utils.focused(), 0,   0,   1,   1)   end,
    ["win.center"]      = function() s(utils.focused(), 0.1, 0.1, 0.8, 0.8) end,
    ["win.balance"]     = function() balanceWindows(gaps) end,
    ["win.undo"]        = function()
